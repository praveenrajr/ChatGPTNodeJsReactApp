...
async function handleDrop(column) {
  ...
  setCurrentPlayer(currentPlayer === boardSettings.colors.p1 ? boardSettings.colors.p2 : boardSettings.colors.p1);
}
      ...
      function isVerticalWin() {
        ...
        for (let column = 0; column < columns; column++) {
          for (let row = 0; row < rows - 3; row++) {
            ...
            if (
              board[index + columns] === color &&
              board[index + columns * 2] === color &&
              board[index + columns * 3] === color
            ) {
              return {
                winner: color,
                winType: winTypes.vertical,
                winningCells: [
                  { row, column },
                  { row: row + 1, column },
                  { row: row + 2, column },
...
<>
      ...
      {!win && (
        ...
        <h2 style={{ color: currentPlayer }}>
          {currentPlayer === boardSettings.colors.p1 ? `${name}'s move` : "Player 2's move"}
        </h2></div>
      )}
      {win && (
        <>
